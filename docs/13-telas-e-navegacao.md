...
| **Postagens** | Lista da conta ativa, com filtros por status e período; fila de pendências. A que falhou mostra a causa em uma linha, em vermelho, até alguém decidir (ADR 0007). O rascunho composto pelo assistente leva a marca **"Composta pelo assistente"**, aqui e na composição (parte 1f) | — | RF-E06, RF-A09, RF-F07, RF-K06 | Filtros numa folha deslizante |
...
| **Perfil** | Identidade e permissões efetivas; Segurança (senha, duas etapas, códigos de recuperação) num bloco só; aparelhos conectados; **Notificações** — o estado deste aparelho (ativas, desligadas, bloqueadas com o caminho para liberar, iPhone fora da tela de início, sem suporte, servidor sem chave), ativar com push de teste, testar, desativar, e um interruptor por tipo que vale para todos os aparelhos; **Instalar o PostIt**, com o caminho do sistema de quem lê; **Aplicativos conectados** (parte 1f) — o assistente conectado, desde quando, último uso, e revogar | — | RF-H07, RF-J03, RF-J04, RF-K05 | Duas colunas no computador. Cada bloco **diz o estado** antes de oferecer o botão, e o formulário abre na linha. "Instalar o PostIt" e "Ativar notificações" ficam aqui, na Fase 1 |
...
| **Assistentes conectados** (parte 1f) | Os acessos de assistente de todos, com dono, desde quando e último uso; revogar | Super admin, com confirmação recente | RF-K05 | Lista, revogar na linha |
| **Autorizar o assistente** (parte 1f) | A tela que o ChatGPT abre: login com as duas etapas e o consentimento — o que o assistente poderá (compor rascunhos) e não poderá (enviar para revisão, aprovar, agendar, publicar) | `POSTAGEM_EDITAR` | RF-K01 | Tela cheia, sem a casca: é aberta de fora, por outro app |

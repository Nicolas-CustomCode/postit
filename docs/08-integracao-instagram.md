...
| V-30 | A Meta publica duas vezes o mesmo container? | Nada. Só diz que o container expira em 24 horas e que o estado vira `PUBLISHED` | O publicador nunca cria outro container depois de pedir a publicação, e confere o estado antes de repetir. Resta a janela de duas chamadas com o **mesmo** container, uma delas muito atrasada | Publicar um container e chamar `media_publish` de novo com o mesmo `creation_id`: observar o erro, ou a segunda mídia |
| V-31 | Retomar um carrossel reaproveitando os contêineres filhos | Nada: não diz se um filho pode entrar num segundo pai, nem por quanto tempo serve. **Observado em 25/09/2026**: o worker reiniciou (commit no dev) depois de criar os dois filhos; a retomada os reaproveitou (`FINISHED`), e o pai novo voltou `status_code: ERROR` **0,2 s depois de criado**, sem texto de diagnóstico. Reagendado, com filhos e pai novos, o mesmo carrossel publicou de primeira. Em 23/09 uma retomada parecida tinha publicado (roteiro, item 10) | A falha é segura — `FALHOU`, sem duplicar, esperando decisão humana (ADR 0007). Hipótese a confirmar: a primeira execução chegou a criar um pai na Meta antes de morrer, sem registrar, e o filho não entra em dois pais | Repetir de propósito: matar o worker logo depois de o pai ser criado na Meta, e antes; se o pai novo sempre falhar quando os filhos já foram usados, a retomada passa a recriar os filhos quando o pai vier `ERROR` |
...
| # | Questão | O que a documentação diz | Postura adotada | Como verificar |
|---|---|---|---|---|
| V-13 | `singletonKey` do pg-boss impede duas tarefas da mesma postagem? | **Resolvido em 22/09/2026** (pg-boss 12.33.1) — **não, sozinho não impede** | Numa fila `standard`, duas tarefas com a mesma chave **entram as duas**: o índice único só existe nas filas com política (`short`, `singleton`, `stately`, `exclusive`). A `publicar-instagram` nasce **`exclusive`** — uma tarefa por chave enquanto criada, em retentativa ou ativa, e a chave volta a valer depois de concluída ou falhada, o que o reagendamento de uma `FALHOU` precisa. A duplicata não lança: `send()` devolve `null`, e quem abriu a transação precisa conferir. A política só vale na criação da fila, então o worker a confere ao subir e se recusa a rodar com outra. Provado em `apps/api/src/queues/boss-policy.integration.spec.ts`. Ver [09](09-motor-agendamento.md#idempotência) | — |

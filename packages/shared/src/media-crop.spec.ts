...
/**
 * O recorte para caber no feed (RF-B02).
 *
 * O caso que motiva tudo: foto de celular em pé, 3024×4032. Ela é 3:4 e o feed
 * aceita no máximo 4:5 — sem recorte, a foto mais comum que existe é recusada.
 */
describe("recorte para o feed", () => {
  const CELULAR_EM_PE = { width: 3024, height: 4032 };

  describe("targetRatioFor — precisa recortar?", () => {
    it("não mexe no que já cabe", () => {
      expect(targetRatioFor(1080, 1080)).toBeNull();
      expect(targetRatioFor(1080, 1350)).toBeNull(); // exatamente 4:5
      expect(targetRatioFor(4032, 3024)).toBeNull(); // paisagem 4:3
    });

    it("foto em pé vira 4:5, a proporção mais próxima", () => {
      expect(targetRatioFor(CELULAR_EM_PE.width, CELULAR_EM_PE.height)).toBeCloseTo(0.8);
    });

    it("panorâmica vira 1.91:1", () => {
      expect(targetRatioFor(4000, 1000)).toBeCloseTo(1.91);
    });

    it("medida inválida não vira recorte", () => {
      expect(targetRatioFor(0, 100)).toBeNull();
      expect(targetRatioFor(100, 0)).toBeNull();
    });
  });
